...
## 다음 할 일 (우선순위 순)
1. ~~홈 화면 UI~~ — 완료 (오늘의 기록 버튼, 읽는 중인 책 카드, 스트릭 표시)
2. ~~빠른 기록 화면~~ — 완료 (진행률 슬라이더 + 페이지 수 선택 입력, 저장 시 홈으로 복귀. 100% 도달 시 자동 완독 처리)
3. 책 상세 화면 — 메모/독후감 리스트, 진행률 슬라이더
4. 백업/복원 (SAF 기반 JSON export/import)
5. 뱃지/업적 로직 및 화면
6. 이북리더기 모드 토글 (설정 화면)

참고: 아직 "책 등록/책장" 화면이 없어서 지금은 읽는 중인 책이 하나도 없으면 홈의 "오늘의 기록" 버튼이 안내 토스트만 띄움. 책 등록 플로우는 3번(책 상세) 작업 시 같이 고려할 것.
